...
export const ESG_ANSWER_TEMPLATES = [
    ...
    // Scope 3 specific
    {
        ...
        generate: (dm) => {
            ...
            if (s3) {
                ...
                if (cats)
                    parts.push(`Categories reported: ${cats}.`);
            }
            if (travel)
    ...
    // POLICY: Human rights
    {
        ...
        generate: (dm) => {
            ...
            if (fte)
                parts.push(`These commitments apply to all ${fmt(fte)} employees.`);
            return parts.join(' ');
        },
    },
    // MEASURE: Fair wages and working conditions
    {
        ...
        generate: (dm) => {
            ...
            if (fte)
                parts.push(`These practices apply to all ${fmt(fte)} FTE employees.`);
            return parts.join(' ');
        },
    },
    // ===================================================================
    ...
    // KPI: H&S incident rates
    {
        ...
        generate: (dm) => {
            ...
            if (trir === 0 && lti === 0 && fat === 0 && !has(dm, 'trir'))
                return null;
            const period = str(dm, 'reportingPeriod');
            const periodStr = period ? ` during ${period}` : ' during the reporting period';
            const parts = [];
    ...
    // ===================================================================
    {
        ...
        generate: (dm) => {
            ...
            if (country)
                answer += ` Our headquarters are located in ${country}.`;
            if (period)
    ...
    // Business travel / commuting (Scope 3)
    {
        ...
        generate: (dm) => {
            ...
            if (!s3 && !travel && !commute) {
                ...
                if (fte)
                    parts.push(`With ${fmt(fte)} employees, we are implementing a travel policy that prioritizes video conferencing and sustainable transport options.`);
            }
    ...
    // Sustainability reporting
    {
        ...
        generate: (dm) => {
            ...
            if (period)
                parts.push(`We are using ${period} as our baseline year for systematic ESG data collection.`);
