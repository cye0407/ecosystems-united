...
export const GAP_ANSWER_TEMPLATES: AnswerTemplate[] = [
...

  {
    ...
    generate: (dm) => {
      ...
      const parts: string[] = [];
      if (farm) parts.push(`${farm} is a ${type || 'farming'} operation${country ? ` based in ${country}` : ''}.`);
      if (ha > 0) parts.push(`The farm covers ${fmt(ha)} hectares.`);
      if (crops) parts.push(`Primary crops: ${crops}.`);
...

  {
    ...
    generate: (dm) => {
      ...
      if (auditDate) parts.push(`The most recent internal audit was completed on ${auditDate}.`);
      if (findings) parts.push(`Key findings: ${findings}.`);
...

  {
    ...
    generate: (dm) => {
      ...
      if (types) parts.push(`Primary soil types on-farm: ${types}.`);
      if (analysisDate) parts.push(`Soil analysis was last conducted on ${analysisDate}.`);
      ...
      if (erosion) parts.push(`Erosion control measures include: ${erosion}.`);
      return parts.join(' ');
    },
  },
...

  {
    ...
    generate: (dm) => {
      ...
      if (types) parts.push(`Fertiliser types used: ${types}.`);
      if (totalKg > 0) parts.push(`Total fertiliser applied during the reporting period: ${fmt(totalKg)} kg.`);
      if (orgPct > 0) parts.push(`${fmt(orgPct)}% of fertiliser applied is from organic sources (compost, manure).`);
      parts.push('All fertiliser applications are recorded with date, field/plot, product name, quantity, method, and operator.');
      if (storage === 'Compliant') parts.push('Fertiliser is stored in a dedicated, covered area separated from crop protection products, with organic and inorganic fertilisers segregated.');
      return parts.join(' ');
    },
  },

  {
    ...
    generate: (dm) => {
      ...
      if (orgPct > 0) parts.push(`Organic sources account for ${fmt(orgPct)}% of total fertiliser inputs.`);
      if (types) parts.push(`Sources include: ${types}.`);
...

  {
    ...
    generate: (dm) => {
      ...
      if (source) parts.push(`Water sources: ${source}.`);
      if (m3 > 0) parts.push(`Total water use during the reporting period: ${fmt(m3)} m\u00B3.`);
      if (method) parts.push(`Primary irrigation method: ${method}.`);
      if (testDate) parts.push(`Water quality was last tested on ${testDate}.`);
      if (results) parts.push(`Results: ${results}.`);
...

  {
    ...
    generate: (dm) => {
      ...
      if (rotation) parts.push(`Crop rotation plan: ${rotation}.`);
...

  {
    ...
    generate: (dm) => {
      ...
      if (products) parts.push(`Products currently in use: ${products}.`);
      ...
      if (storage === 'Compliant') parts.push('PPPs are stored in a secure, well-ventilated, locked facility with appropriate signage, spill containment, and inventory management.');
      return parts.join(' ');
    },
  },

  {
    ...
    generate: (dm) => {
      ...
      if (mrlDate) parts.push(`Residue analysis was last conducted on ${mrlDate}.`);
      if (mrlResults) parts.push(`Results: ${mrlResults}.`);
...

  {
    ...
    generate: (dm) => {
      ...
      const parts: string[] = [];
      if (method) parts.push(`Harvesting is carried out using the following method: ${method}.`);
      ...
      if (hygiene === 'In place') parts.push('A hygiene protocol is in place covering worker personal hygiene, cleaning schedules for equipment and containers, and prevention of cross-contamination.');
      if (packing === 'Available') parts.push('An on-site packing facility is available with documented cleaning and sanitation procedures.');
      if (cold === 'In place') parts.push('Cold chain management is in place from harvest through to dispatch, with temperature monitoring and records maintained.');
      if (foreign === 'In place') parts.push('Foreign body prevention measures are implemented, including visual inspections and, where appropriate, detection equipment.');
      return parts.join(' ');
    },
  },
...

  {
    ...
    generate: (dm) => {
      ...
      if (riskAssessment === 'Completed') parts.push('A food safety risk assessment has been completed covering biological, chemical, and physical hazards.');
      if (microDate) parts.push(`Microbiological testing was last conducted on ${microDate}.`);
      if (microResults) parts.push(`Results: ${microResults}.`);
...

  {
    ...
    generate: (dm) => {
      ...
      const parts: string[] = [];
      if (count > 0) parts.push(`The farm employs ${fmt(count)} workers.`);
      ...
      if (training === 'Maintained') parts.push('Training records are maintained for all workers, including induction training, task-specific competence, and PPP handling where applicable.');
      if (ppe === 'Yes') parts.push('Appropriate personal protective equipment (PPE) is provided to all workers based on risk assessment and product label requirements.');
      if (firstAid === 'Available') parts.push('First aid kits are available at all work locations and regularly checked.');
      if (accidents === 'Maintained') parts.push('Accident and incident records are maintained with investigation findings and corrective actions.');
      return parts.join(' ');
    },
  },

  {
    ...
    generate: (dm) => {
      ...
      const parts: string[] = [];
      if (welfare === 'Provided') parts.push('Worker welfare facilities are provided including clean toilets, handwashing stations, drinking water, and designated rest areas, all within reasonable proximity to the work area.');
      if (hours === 'Compliant') parts.push('Working hours comply with national legislation and applicable collective agreements. Overtime is voluntary and compensated in accordance with the law.');
      if (child === 'Confirmed') parts.push('The farm confirms that no child labour is employed. Workers below 18 years are not engaged in hazardous work.');
      if (subs === 'In place') parts.push('Subcontractor and agency worker agreements include clauses on health, safety, welfare, and compliance with labour legislation.');
      if (parts.length === 0) parts.push('Worker welfare provisions and working conditions comply with national legislation and GlobalG.A.P. GRASP requirements.');
      return parts.join(' ');
    },
  },
...

  {
    ...
    generate: (dm) => {
      ...
      const parts: string[] = [];
      if (policy === 'In place') parts.push('An environmental policy is in place covering biodiversity, waste, energy, water, and pollution prevention.');
      if (bio === 'In place') parts.push('A biodiversity action plan has been developed, including identification of priority habitats, conservation of ecological infrastructure (hedgerows, buffer zones, water courses), and monitoring of wildlife indicators.');
      if (waste === 'In place') parts.push('A waste management plan is in place covering identification, segregation, and responsible disposal of all waste streams.');
      if (wasteKg > 0) {
        const tonnes = Math.round(wasteKg / 100) / 10;
        parts.push(`Total waste during the reporting period: ${fmt(tonnes)} tonnes.`);
      }
      if (recycling > 0) parts.push(`Current recycling rate: ${fmt(recycling)}%.`);
      if (energy > 0) parts.push(`Energy consumption: ${fmt(energy)} kWh.`);
      if (renewable > 0) parts.push(`${fmt(renewable)}% of energy is from renewable sources.`);
...

  {
    ...
    generate: (dm) => {
      ...
      const parts: string[] = [];
      if (procedure === 'Documented') parts.push('A documented complaint procedure is in place for receiving, recording, and resolving customer complaints.');
      if (log === 'Maintained') parts.push('A complaint log is maintained with details of each complaint, investigation, root cause analysis, and corrective actions taken.');
...

  {
    ...
    generate: (dm) => {
      ...
      if (batch === 'In place') parts.push('Each package carries a unique batch/lot number enabling full traceability.');
