...
export interface ESGCompanyData {
  ...
  // Emissions (user-provided overrides)
  scope1Tco2e?: number;
  scope2Tco2e?: number;
  scope3Tco2e?: number;
  ...
  // Structured policies (from Passport policy tracker)
  policies?: Array<{
    id: string;
    name: string;
    category: 'environmental' | 'social' | 'governance';
    exists: boolean;
    status: string; // 'available' | 'in_progress' | 'not_available' | 'not_planned'
    isCertification?: boolean;
  }>;

  // Structured documents (from Passport evidence registry)
  documents?: Array<{
    name: string;
    category: string; // 'certificate' | 'policy' | 'audit' | 'evidence' | 'report' | 'other'
    validUntil?: string;
    isValid?: boolean; // true if not expired or no expiry date
  }>;
}
...
export function esgRetrieveData(
  ...
  for (const domain of allDomains) {
    switch (domain) {
      ...
      case 'transport': {
        ...
        if (data.businessTravelKm) operational.push({ domain: 'transport', field: 'businessTravel', label: 'Business Travel', value: data.businessTravelKm, unit: 'km', confidence: 'high' });
        if (data.employeeCommuteKm) operational.push({ domain: 'transport', field: 'employeeCommute', label: 'Employee Commuting', value: data.employeeCommuteKm, unit: 'km', confidence: 'high' });
        if (data.freightTonKm) operational.push({ domain: 'transport', field: 'freightTransport', label: 'Freight Transport', value: data.freightTonKm, unit: 'ton-km', confidence: 'high' });
  ...
  const POLICY_CATEGORY_DOMAINS: Record<string, string[]> = {
  ...
  const CERT_DOMAIN_MAP: [string, string[]][] = [
