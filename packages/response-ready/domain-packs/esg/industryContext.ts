...
import type { IndustryContext, IndustryContextProvider } from '../../src/types/domain-pack';
...
const MANUFACTURING: IndustryTerminology = {
...
const LOGISTICS: IndustryTerminology = {
...
const TEXTILES: IndustryTerminology = {
...
const CHEMICALS: IndustryTerminology = {
  industry: 'Chemicals',
  terms: {
...
const ELECTRONICS: IndustryTerminology = {
...
const FOOD: IndustryTerminology = {
...
const CONSTRUCTION: IndustryTerminology = {
...
const SERVICES: IndustryTerminology = {
...
const MEASURES_MAP: Record<string, MeasuresLibrary> = {
  'manufacturing': manufacturingData as MeasuresLibrary, 'metal fabrication': manufacturingData as MeasuresLibrary,
  'automotive': manufacturingData as MeasuresLibrary, 'industrial': manufacturingData as MeasuresLibrary,
  'aerospace': manufacturingData as MeasuresLibrary, 'mining': manufacturingData as MeasuresLibrary,
  'transport': logisticsData as MeasuresLibrary, 'logistics': logisticsData as MeasuresLibrary,
  'shipping': logisticsData as MeasuresLibrary, 'freight': logisticsData as MeasuresLibrary,
  'textile': textilesData as MeasuresLibrary, 'apparel': textilesData as MeasuresLibrary,
  'garment': textilesData as MeasuresLibrary, 'fashion': textilesData as MeasuresLibrary,
  'chemical': chemicalsData as MeasuresLibrary,
  'electronic': electronicsData as MeasuresLibrary, 'technology': electronicsData as MeasuresLibrary,
  'software': electronicsData as MeasuresLibrary, 'telecommunication': electronicsData as MeasuresLibrary,
  'food': foodAgricultureData as MeasuresLibrary, 'agriculture': foodAgricultureData as MeasuresLibrary,
  'beverage': foodAgricultureData as MeasuresLibrary,
  'construction': constructionData as MeasuresLibrary, 'building': constructionData as MeasuresLibrary,
  'financial': servicesData as MeasuresLibrary, 'healthcare': servicesData as MeasuresLibrary,
  'pharma': servicesData as MeasuresLibrary, 'professional service': servicesData as MeasuresLibrary,
  'retail': servicesData as MeasuresLibrary, 'wholesale': servicesData as MeasuresLibrary,
  'real estate': servicesData as MeasuresLibrary, 'energy': servicesData as MeasuresLibrary,
  'utilities': servicesData as MeasuresLibrary, 'hospitality': servicesData as MeasuresLibrary,
  'tourism': servicesData as MeasuresLibrary, 'education': servicesData as MeasuresLibrary,
  'other': servicesData as MeasuresLibrary,
};
...
function findContext(industry: string): IndustryTerminology {
...
function findMeasures(industry: string): MeasuresLibrary {
  const key = industry.toLowerCase();
  for (const [pattern, lib] of Object.entries(MEASURES_MAP)) {
    if (key.includes(pattern)) return lib;
  }
  return servicesData as MeasuresLibrary;
}
...
export const esgIndustryContextProvider: IndustryContextProvider = {
  ...
  getMeasures(industry: string, topic: string, subcategory: string, count: number = 3): string[] {
  ...
  getPolicyLanguage(industry: string, topic: string, style: string, year?: string): string | null {
    ...
    if (year) text = text.replace(/{year}/g, year);
    else text = text.replace(/{year}/g, '2025');
    return text;
  },
};
...
export function domainToTopic(domain: string): string | null {
