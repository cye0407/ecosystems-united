...
export function val(dataMap: Map<string, RetrievedDataPoint>, field: string): string | number | boolean | null {
  const p = dataMap.get(field);
  return p?.value ?? null;
}

export function has(dataMap: Map<string, RetrievedDataPoint>, ...fields: string[]): boolean {
...
export function num(dataMap: Map<string, RetrievedDataPoint>, field: string): number {
  const v = val(dataMap, field);
  return typeof v === 'number' ? v : 0;
}

export function str(dataMap: Map<string, RetrievedDataPoint>, field: string): string {
  const v = val(dataMap, field);
...
export function buildDataMap(context: DataContext): Map<string, RetrievedDataPoint> {
...
function applyRewriteRules(text: string, rules: ScrubRule[]): string {
  let result = text;
...
function findMatchingTemplate(
  ...
  const candidates = templates.filter(t => {
    ...
    if (!t.topics.some(topic => matchResult.topics.includes(topic))) return false;
    // If template declares questionTypes, must match the classified type
    if (t.questionTypes && questionType && !t.questionTypes.includes(questionType)) return false;
    return true;
  });
  ...
  return candidates.sort((a, b) => {
    ...
    if (bPrimaryOverlap !== aPrimaryOverlap) return bPrimaryOverlap - aPrimaryOverlap;
    ...
    if (Math.abs(bSpecificity - aSpecificity) > 0.01) return bSpecificity - aSpecificity;
...
function getFrameworkNote(
  framework: string | undefined,
  frameworkNotes: Record<string, string> | undefined
): string {
  if (!framework || !frameworkNotes) return '';
...
export function createAnswerGenerator<TProfile = Record<string, unknown>>(
  deps: AnswerGeneratorDeps<TProfile>
): AnswerGeneratorInstance<TProfile> {
  const {
    templates,
    frameworkNotes,
    fieldToMetricKey = {},
  ...
  function generateSimpleAnswer(
    ...
    if (profile && questionType && maturityResolver && matrixGenerator) {
      ...
      if (shouldUseMatrix) {
        const matrixAnswer = matrixGenerator.generate(
    ...
    if (profile && informalPracticeHandler) {
      ...
      if (relevant.length > 0) {
        const answer = informalPracticeHandler.generateAnswer(
          '', // company name — pack should resolve from profile
          relevant,
          matchResult,
          '', // industry — pack should resolve from profile
          framework
        );
  ...
  function generateAnswerDraft(
    ...
    const { answer, dataValue, dataSource, usedPractice } = generateSimpleAnswer(
      dataContext, matchResult, framework, profile, questionType
    );
    ...
    const hasEstimates = dataContext.calculated.some(p =>
      p.label.toLowerCase().includes('estimate') ||
    ...
    const metricKeysUsed = [...new Set(
      allPoints.map(p => fieldToMetricKey[p.field]).filter((k): k is string => !!k)
    )];
    ...
    const promptForMissing: string | undefined = matchResult.csvPromptIfMissing || undefined;
  ...
  function generateAnswerDrafts(
    ...
    return questions.map((q, i) =>
      generateAnswerDraft(q, matchResults[i], dataContexts[i], config, profile, classifications?.[i])
    );
  }
