...
function scoreQuestion(
  ...
  for (const rule of signalRules) {
    if (!scores[rule.type]) scores[rule.type] = { score: 0, signals: [] };
...
export interface ClassifierInstance {
  ...
  classifyQuestions: (questions: Array<{ id: string; text: string; category?: string }>) => ClassificationResult[];
...
export function createClassifier(
  signalRules: SignalRule[],
  questionTypes: string[],
  defaultType: string = questionTypes[0] || 'UNKNOWN'
): ClassifierInstance {
  ...
  function getClassificationStats(results: ClassificationResult[]) {
    const byType: Record<string, number> = {};
    for (const t of questionTypes) byType[t] = 0;
