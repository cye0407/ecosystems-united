...
import type { AnswerDraft, ExportSheetConfig, ExportMetadata, SheetData } from '../types';
...
function styleHeaderCells(ws: ExcelJS.Worksheet, rowNum: number, colCount: number): void {
...
function styleDataCells(ws: ExcelJS.Worksheet, rowNum: number, colCount: number, isEven: boolean): void {
  for (let i = 0; i < colCount; i++) {
    const c = cell(ws, rowNum, START_COL + i);
...
function styleConfidenceCell(c: ExcelJS.Cell, value: string): void {
...
function styleSectionCells(ws: ExcelJS.Worksheet, rowNum: number, colCount: number): void {
  const row = ws.getRow(rowNum);
  row.height = 24;
  for (let i = 0; i < colCount; i++) {
    const c = cell(ws, rowNum, START_COL + i);
...
function setupWorksheet(workbook: ExcelJS.Workbook, name: string, opts: {
  columnWidths: number[];
  ...
  const ws = workbook.addWorksheet(name, {
    ...
    views: [{
      ...
      ...(opts.freezeHeader !== false ? { state: 'frozen' as const, ySplit: headerRow, xSplit: 0 } : {}),
    }],
  });
...
function buildDefaultAnswersSheet(drafts: AnswerDraft[]): SheetData {
  const headers = [
...
function buildDefaultReviewSheet(drafts: AnswerDraft[]): SheetData {
...
function writeRow(ws: ExcelJS.Worksheet, rowNum: number, data: (string | number | boolean | null)[]): void {
  for (let i = 0; i < data.length; i++) {
    const val = data[i];
    if (val !== null && val !== undefined) {
      cell(ws, rowNum, START_COL + i).value = val as ExcelJS.CellValue;
    }
  }
}

function applySheetData(workbook: ExcelJS.Workbook, sheetName: string, sheetData: SheetData): void {
  ...
  for (const rowData of sheetData.rows) {
    const isSectionSep = typeof rowData[1] === 'string' && (rowData[1] as string).startsWith('---');
...
function applySummaryStyle(workbook: ExcelJS.Workbook, sheetName: string, sheetData: SheetData): void {
  const ws = setupWorksheet(workbook, sheetName, {
    columnWidths: sheetData.columnWidths || [],
    freezeHeader: false,
  });
  ...
  for (const rowData of sheetData.rows) {
    writeRow(ws, currentRow, rowData as (string | number | boolean | null)[]);
    ...
    if (firstCell.startsWith('  ')) {
      ...
      if (colCount > 1) cell(ws, currentRow, START_COL + 1).font = { size: 10, color: { argb: COLORS.ink } };
      currentRow++;
      continue;
    }
...
function buildFileName(metadata: ExportMetadata, customName?: string): string {
 ...
 */
export async function exportToBuffer(opts: Omit<ExportOptions, 'fileName'>): Promise<Uint8Array> {
