// ============================================
// ResponseReady — DomainPack Interface
// ============================================
// This is the contract between the engine and any domain implementation.
// Implement this interface to add support for a new questionnaire domain
// (ESG, security, RFP, ISO, etc.).
...
export interface DomainPack<TData = Record<string, unknown>, TProfile = Record<string, unknown>> {
  ...
  // ------------------------------------------
  // Context Providers (optional — for sophisticated generation)
  // ------------------------------------------
  ...
  // ------------------------------------------
  // Domain-Specific Calculations (optional)
  // ------------------------------------------
...
export interface MaturityResolver<TProfile = Record<string, unknown>> {
  resolve: (
    ...
    hasData: boolean
  ) => string; // Returns maturity band name, e.g. 'none' | 'informal' | 'formal'
}
...
export interface Calculator<TData = Record<string, unknown>> {
  calculate: (data: TData, ...args: unknown[]) => { value: number; unit: string; source: string } | null;
}
