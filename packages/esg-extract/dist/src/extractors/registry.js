 ...
 */
function extractByType(text, docType, config) {
    switch (docType) {
 ...
 */
function extractBestEffort(text, config) {
    const results = [
...
function attachDetection(result, detection) {
 ...
 */
export async function extractFromPdf(buffer, config) {
 ...
 * Auto-detects document type and routes to the right extractor.
 */
export function extractFromText(text, config) {
    const detection = config?.forceType
        ? { type: config.forceType, score: 999, runnerUp: undefined, runnerUpScore: undefined }
