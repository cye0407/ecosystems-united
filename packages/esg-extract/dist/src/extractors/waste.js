 ...
 */
export function extractWaste(text, config) {
    ...
    if (total && recycled && !rateField && typeof total.value === 'number' && typeof recycled.value === 'number' && total.value > 0) {
        fields.push({
            field: 'recyclingRate',
            value: Math.round(recycled.value / total.value * 10000) / 100,
            unit: '%',
            confidence: 'medium',
            score: 0.7,
            ...
            rawUnitText: '%',
            normalizedValue: Math.round(recycled.value / total.value * 10000) / 100,
            normalizedUnit: '%',
    ...
    const gaps = [];
    const issues = [];
    if (!fields.some(field => field.field === 'totalWasteKg'))
        gaps.push('totalWasteKg');
