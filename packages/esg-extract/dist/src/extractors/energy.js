...
function detectEnergyType(text) {
 ...
 */
export function extractEnergy(text, config) {
    ...
    const finalDocType = (hasWater && hasWaterOnly) ? 'water_bill' : documentType;
    // Detect gaps
    const gaps = [];
    const issues = [];
