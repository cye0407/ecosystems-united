...
function deriveDieselLitersFromPriceAndAmount(text) {
    ...
    if (!unitPriceMatch || (!subtotalMatch && !totalMatch))
        return undefined;
    ...
    if (!unitPrice || !amount || unitPrice <= 0)
        return undefined;
    return {
        field: 'dieselLiters',
...
export function extractFuel(text, config) {
    ...
    if (!directDiesel && derivedDiesel) {
        ...
        const directValue = directDiesel.value;
        const derivedValue = derivedDiesel.value;
