 ...
 */
export function extractWorkforce(text, config) {
    ...
    if (total && female && typeof total.value === 'number' && typeof female.value === 'number' && total.value > 0) {
        fields.push({
            field: 'femalePercent',
            value: Math.round(female.value / total.value * 10000) / 100,
            unit: '%',
            confidence: 'medium',
            score: 0.7,
            ...
            rawUnitText: '%',
            normalizedValue: Math.round(female.value / total.value * 10000) / 100,
            normalizedUnit: '%',
    ...
    const gaps = [];
    const issues = [];
    if (!fields.some(f => f.field === 'totalEmployees'))
        gaps.push('totalEmployees');
